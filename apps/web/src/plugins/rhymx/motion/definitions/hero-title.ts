...
const HERO_PARAMS: ParamDefinition<keyof HeroParams & string>[] = [
	{
		...
		label: "Title",
		type: "text",
		default: "Big ideas, told simply",
	},
	{
		key: "subtitle",
		label: "Subtitle",
		type: "text",
		default: "A short documentary film",
	},
...
function renderHero({
	...
	lines.forEach((line, index) => {
		ctx.fillText(
			line,
			width * 0.14 + 60 * (1 - enter),
			height * 0.5 + index * height * 0.13,
		);
	});
	...
	if (subtitle) {
		...
		ctx.fillText(
			subtitle,
			width * 0.14 + 80 * (1 - enter),
			height * 0.5 + lines.length * height * 0.13 + 18,
		);
	}
	ctx.restore();
}
