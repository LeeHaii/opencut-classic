...
function renderKineticTitle({ ctx, params, width, height, localTime = 0, durationSec = 3 }: GraphicRenderContext): void {
	ctx.clearRect(0, 0, width, height);
	const color = accent({ params });
	const exit = exitFactor({ localTime, durationSec });
	const words = textParam({ params, key: "title" }).split(/\s+/).filter(Boolean);
	...
	const totalWidth =
		wordWidths.reduce((sum, w) => sum + w, 0) + Math.max(0, words.length - 1) * spaceWidth;

	ctx.save();
	ctx.globalAlpha = exit;
	let cursorX = width / 2 - totalWidth / 2;
	words.forEach((word, index) => {
		const enter = easeOutBack({ t: (localTime - index * WORD_STAGGER_SEC) / 0.55 });
		if (enter > 0) {
			ctx.save();
